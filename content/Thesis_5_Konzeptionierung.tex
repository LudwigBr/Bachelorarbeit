...
Damit die Multicopter an der Ladestation geladen werden können müssen diese von den Warteplätzen auf die Ladeplätze verschoben werden. Dafür muss bei jedem Aktualisierungsvorgang die Platzvergabe geprüft und gegebenenfalls geändert werden. Direkt nach der Aufladung werden die bis zu ihrem Zielzustand aufgeladenen Multicopter von den Ladeplätzen entfernt und in die entsprechende Liste verschoben. Im Anschluss daran werden die leeren Ladeplätze mit wartenden Multicoptern gefüllt. Im letzten Schritt können nachträgliche Änderungen an der Ladereihenfolge vorgenommen werden und die Ladeplätze mit anderen Multicoptern gefüllt werden. 
Die Auswahl des nächsten zu ladenden Multicopters ist in eine andere Methode getNextWaitingObjectIterator() ausgelagert. Diese Auslagerung soll bei einem komplexen Auswahlalgorithmus die Gesamtkomplexität der Anwendung reduzieren. Im trivialsten Fall wird hier nur das erste Objekt aus dem Warteplätzecontainer ausgewählt.
...
Das Interface für den Ladealgorithmus stellt die Information bereit, bis zu welcher relativen Kapazität der Ladevorgang effizient ist. Für einen durchgehend linearen Algorithmus wäre der Wert 100\% und die Teilaufladungen wären unnötig. Im genutzten CCCV Verfahren wird der Akkumulator in der ersten Phase bedeutend schneller geladen. Damit insgesamt in der Simulation eine größere Kapazität in kürzerer zeit aufgeladen werden kann, kann es sinnvoll sein zu erst den ersten linearen Teil aller wartenden Multicopter zu laden und erst im Anschluss die langsamere zweite Phase der Multicopter durchzuführen. 
Diese Funktionalität sollte konfigurierbar sein, denn unter Umständen werden voll geladene Multicopter für einzelne Missionen gebraucht, die anders nicht ausführbar sind. Insofern die Konfiguration aktiviert ist wird nach der Aufladung geprüft, ob einer der gerade geladenen Akkumulatoren die erste Phase überschritten hat und eine Alternative bereit zur Ladung ist. Sollte dieser Fall eintreffen werden die beiden zugehörigen Multicopter getauscht. 
Durch den unvorhersehbaren Tausch sind die getroffenen Vorhersagen nicht mehr genau und können unter Umständen nicht eingehalten werden. Wobei weiterhin zum zugesagten Zeitpunkt wenigstens die lineare Phase abgeschlossen ist und der Multicopter je nach verwendeter Ladestromstärke einen Großteil seiner Gesamtkapazität zur Verfügung stehen hat.
